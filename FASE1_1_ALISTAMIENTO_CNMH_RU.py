...
cnmh = pd.merge(cnmh, dane, how='left', left_on=['depto_caso', 'muninicio_caso'],
...
cnmh["fecha_ocur_anio"] = np.where(cnmh["fecha_ocur_anio"].astype(float) < 1900, np.nan, cnmh["fecha_ocur_anio"])
cnmh["fecha_ocur_mes"] = np.where(cnmh["fecha_ocur_mes"] == "00", np.nan, cnmh["fecha_ocur_mes"])
cnmh["fecha_ocur_dia"] = np.where((cnmh["fecha_ocur_dia"] == "00") | (cnmh["fecha_ocur_dia"] == ""), np.nan, cnmh["fecha_ocur_dia"])
# Validaciones
meses = ["01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12"]
dias = ["01", "02", "03", "04", "05", "06", "07", "08", "09"] + list(map(str, range(10, 32)))

cnmh = cnmh[cnmh["fecha_ocur_anio"].astype(str).isin(map(str, range(1900, 2023)))]
cnmh = cnmh[cnmh["fecha_ocur_mes"].astype(str).isin(meses)]
cnmh = cnmh[cnmh["fecha_ocur_dia"].astype(str).isin(dias)]

#presuntos responsables
sin_informacion_actores = [
    'NO DEFINIDO', '', 'NO IDENTIFICA ', 'SIN INFORMACIÓN  CONFLICTO ARMADO',
...
cnmh['espeficicacion_presunto_responsable'] = np.where(
    cnmh['espeficicacion_presunto_responsable'].isin(sin_informacion_actores),
    None,
    cnmh['espeficicacion_presunto_responsable']
)
...
cnmh[['primer_nombre', 'segundo_nombre', 'primer_apellido', 'segundo_apellido']] = cnmh['nombres_apellidos'].apply(lambda x: pd.Series(limpiar_nombres_apellidos(x)))
...
cnmh['documento'] = np.where((cnmh['documento'].str.isnumeric() == False) |
                             ((cnmh['documento'].str.isnumeric()) &
                              (cnmh['tipo_documento'].isin(['TARJETA DE IDENTIDAD', 'CEDULA DE CIUDADANIA']))),
...
resumen_documento.to_csv("log/revision_documentos_cnmh_ru.csv", sep=";", index=False)
# Resumen de las columnas que comienzan con "documento_"
resumen_documento.filter(like='documento_').agg(['sum']).reset_index()
#sexo
...
# Fecha de nacimiento
cnmh["fecha_nacimiento_original"] = pd.to_datetime(cnmh["fecha_nacimiento"])
...
cnmh = cnmh[(cnmh["edad"].between(1, 100, inclusive=True) | cnmh["edad"].isna())]
...
# guardar resultado en la base de datos de destino
...
con2 = pyodbc.connect(conn_str)

# Escribir el DataFrame cnmh_ident en la tabla orq_salida.CNMH_RU
cnmh_ident.to_sql(name='CNMH_RU', con=con2, if_exists='replace', index=False)

# Escribir el DataFrame cnmh_no_ident en la tabla orq_salida.CNMH_RU_PNI
cnmh_no_ident.to_sql(name='CNMH_RU_PNI', con=con2, if_exists='replace', index=False)
