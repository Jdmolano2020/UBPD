...
/****** Object:  StoredProcedure [dbo].[VALIDA_V_CEV_JEP_UNIVERSO]    Script Date: 3/11/2023 2:35:16 p. m. ******/
DROP PROCEDURE [dbo].[VALIDA_V_CEV_JEP_UNIVERSO]
GO
...
-- Description:	Realiza la consulta de las personas desaparecidas de la fuente V_CEV_JEP
-- =============================================
CREATE   PROCEDURE [dbo].[VALIDA_V_CEV_JEP_UNIVERSO]
WITH RECOMPILE 
AS
BEGIN
  SET NOCOUNT ON;
  IF OBJECT_ID('[dbo].[BD_CEV_JEP_VAL]', 'U') IS NOT NULL
    DROP TABLE [dbo].[BD_CEV_JEP_VAL]
  ...
  SELECT A.[tabla_origen]
	  ...
	  INTO [dbo].[BD_CEV_JEP_VAL]
  FROM (SELECT match_group_id AS codigo_unico_fuente,* 
  FROM [ORQ_SALIDA].[CONSOLIDADO_JEP]) C
  LEFT JOIN [dbo].[CEV_JEP_U] A
			ON C.codigo_unico_fuente = A.[codigo_unico_fuente]
		LEFT JOIN [dbo].[CEV_JEP] B 
