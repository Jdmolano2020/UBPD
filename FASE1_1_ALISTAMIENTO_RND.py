...
df_rnd.rename(columns={'DES_DARE_CLASIFICACION_DESAPARICION': 'CLASIFICACION_DESAPARICION',
...
df_rnd['fecha_ocur_anio'] = pd.to_numeric(df_rnd['fecha_ocur_anio'], errors='coerce')
df_rnd['fecha_ocur_mes'] = pd.to_numeric(df_rnd['fecha_ocur_mes'], errors='coerce')
df_rnd['fecha_ocur_dia'] = pd.to_numeric(df_rnd['fecha_ocur_dia'], errors='coerce')
...
df_rnd.loc[(df_rnd['presunto_responsable'] != "") &
       ...
       (df_rnd['pres_resp_paramilitares'].isna() | pd.to_numeric(df_rnd['pres_resp_paramilitares']) == 0) &
       (df_rnd['pres_resp_grupos_posdesmov'].isna() | pd.to_numeric(df_rnd['pres_resp_grupos_posdesmov']) == 0 ) &
       (df_rnd['pres_resp_agentes_estatales'].isna() | pd.to_numeric(df_rnd['pres_resp_agentes_estatales']) == 0) &
       (df_rnd['pres_resp_guerr_farc'].isna() | pd.to_numeric(df_rnd['pres_resp_guerr_farc']) == 0) &
       (df_rnd['pres_resp_guerr_eln'].isna() | pd.to_numeric(df_rnd['pres_resp_guerr_eln']) == 0) &
       (df_rnd['pres_resp_guerr_otra'].isna() | pd.to_numeric(df_rnd['pres_resp_guerr_otra']) == 0), 'pres_resp_otro'] = 1

# Tipo de hecho
# Crear la variable TH_DF
df_rnd['TH_DF'] = 0
df_rnd.loc[(df_rnd['clasificacion_desaparicion'].str.contains("DESAPARICION")) &
       (df_rnd['clasificacion_desaparicion'].str.contains("FORZADA")), 'TH_DF'] = 1
# Crear la variable TH_SE
df_rnd['TH_SE'] = 0
df_rnd.loc[df_rnd['clasificacion_desaparicion'].str.contains("SECUESTRO"), 'TH_SE'] = 1
...
df_rnd.rename(columns={'ancestro_racial': 'iden_pertenenciaetnica'}, inplace=True)
...
df_rnd['anio_nacimiento'] = pd.to_numeric(df_rnd['anio_nacimiento'], errors='coerce')
df_rnd['mes_nacimiento'] = pd.to_numeric(df_rnd['mes_nacimiento'], errors='coerce')
df_rnd['dia_nacimiento'] = pd.to_numeric(df_rnd['dia_nacimiento'], errors='coerce')
...
df_rnd[['edad_1', 'edad_2']] = df_rnd['edad_rangos'].str.split('-', expand=True)
...
df_rnd.loc[(df_rnd['primer_nombre'].str.contains("POLICIA")) |
       ((df_rnd['primer_nombre'] == "POLICIA") &
        (df_rnd['segundo_nombre'].isna() | df_rnd['segundo_nombre'] == "") &
        (df_rnd['primer_apellido'].isna() | df_rnd['primer_apellido'] == "") &
        (df_rnd['segundo_apellido'].isna() | df_rnd['segundo_apellido'] == "")),
       'rni'] = 1
...
df_rnd.loc[((df_rnd['primer_nombre'].isin(["NN", "N", "XX", "X"])) |
        ...
        (df_rnd['segundo_apellido'].isin(["NN", "N", "XX", "X"]))), 'rni'] = 1
...
df_rnd.loc[(df_rnd['clasificacion_desaparicion'] == "DESASTRE NATURAL") |
...
log = {
    ...
    'filas_iniciales_df': nrow_df_rnd_ini,
    'filas_final_df': nrow_df_rnd_fin,
    'filas_df_ident': nrow_df_rnd_ident,
    'filas_df_no_ident': nrow_df_rnd_no_ident,
