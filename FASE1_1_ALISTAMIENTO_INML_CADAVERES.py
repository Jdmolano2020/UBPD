...
df_inml.rename(columns={'var20': 'CAD_DARE_DPTO_EXPEDICION',
...
# Renombrar columna
df_inml.rename(columns={'numero_radicado': 'codigo_unico_fuente'}, inplace=True)
# Origen de los datos
...
df_inml['fecha_ocur_anio'] = df_inml['fecha_ocur_anio'].str.replace('18', '19', n=1)
df_inml['fecha_ocur_anio'] = df_inml['fecha_ocur_anio'].str.replace('179', '197', n=1)
df_inml['fecha_ocur_anio'] = df_inml['fecha_ocur_anio'].str.replace('169', '196', n=1)
df_inml['fecha_ocur_anio'] = df_inml['fecha_ocur_anio'].str.replace('159', '195', n=1)

df_inml['fecha_ocur_anio'] = pd.to_numeric(df_inml['fecha_ocur_anio'], errors='coerce')
df_inml['fecha_ocur_mes'] = pd.to_numeric(df_inml['fecha_ocur_mes'], errors='coerce')
df_inml['fecha_ocur_dia'] = pd.to_numeric(df_inml['fecha_ocur_dia'], errors='coerce')
...
df_inml.rename(columns={'ancestro_racial': 'iden_pertenenciaetnica'}, inplace=True)
...
df_inml[['edad_1', 'edad_2']] = df_inml['rango_edad'].str.split('-', expand=True)
...
df_inml['non_miss'] = df_inml[['primer_nombre', 'segundo_nombre',
                     'primer_apellido',
...
df_inml.loc[((df_inml['primer_nombre'].isin(["NN", "N", "XX", "X"])) |
        ...
        (df_inml['segundo_apellido'].isin(["NN", "N", "XX", "X"]))), 'rni'] = 1
...
nrow_df = len(df_inml)
print("Registros :", nrow_df)#531620
print("Registros NI:", nrow_df_no_ident)#142555

BD_INML_CAD_PNI_file_path = os.path.join(DIRECTORY_PATH, "archivos depurados", "BD_INML_CAD_PNI.csv")
...
with engine.connect() as conn, conn.begin():
    df_inml_rni.to_sql(name=DB_TABLE, con=engine, schema=DB_SCHEMA, if_exists='replace', index=False)
...
df_inml.drop_duplicates(subset=['codigo_unico_fuente'], keep='first', inplace=True)
...
with engine.connect() as conn, conn.begin():
    df_inml.to_sql(name=DB_TABLE, con=engine, schema=DB_SCHEMA, if_exists='replace', index=False)
...
log_file_path = os.path.join(DIRECTORY_PATH, "log", "resultado_df_inml_cad.yaml")
