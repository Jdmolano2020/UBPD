...
# Cargue de datos
query = "select * from orq_salida.FGN_INACTIVOS"
...
variables_a_convertir = ['tabla_origen', 'nombre_completo', 'primer_nombre', 'segundo_nombre', 'primer_apellido',
                         ...
                         'descripcion_relato', 'pais_de_ocurrencia', 'presunto_responsable', 'situacion_actual_des']

df[variables_a_convertir] = df[variables_a_convertir].apply(lambda x: x.str.upper())
...
variables_a_limpiar = ['nombre_completo', 'primer_nombre', 'segundo_nombre', 'primer_apellido', 'segundo_apellido',
                       'nombre_completo', 'presunto_responsable', 'sexo', 'tipo_de_hecho', 'iden_pertenenciaetnica']
...
departamentos = pd.read_stata("fuentes secundarias/tablas complementarias/DIVIPOLA_departamentos_122021.dta")
df = df.merge(departamentos, left_on='codigo_dane_departamento', right_on='codigo_dane_departamento', how='left', suffixes=('', '_departamento'))
df.drop(index=df[df['_merge'] == 2].index, inplace=True)
df.drop(columns=['_merge'], inplace=True)
df.rename(columns={'departamento': 'departamento_ocurrencia'}, inplace=True)
# Realizar un merge con el archivo DIVIPOLA_municipios_122021.dta
municipios = pd.read_stata("fuentes secundarias/tablas complementarias/DIVIPOLA_municipios_122021.dta")
df = df.merge(municipios, left_on='codigo_dane_municipio', right_on='codigo_dane_municipio', how='left', suffixes=('', '_municipio'))
df.drop(index=df[df['_merge'] == 2].index, inplace=True)
df.drop(columns=['_merge'], inplace=True)
...
df['fecha_ocur_mes'].replace('', np.nan, inplace=True)
df['fecha_ocur_dia'].replace('', np.nan, inplace=True)
...
# Formateo de fechas de ocurrencia
for var in ['fecha_ocur_mes', 'fecha_ocur_dia']:
    df[var + '_dtf'] = df[var].apply(lambda x: f"{int(x):02.0f}" if not pd.isna(x) else x)
    df[var] = df[var].replace('', np.nan)
    df.drop(columns=[var + '_dtf'], inplace=True)
# Renombrar y corregir fechas de desaparición
df.rename(columns={'fecha_ocur_anio': 'fecha_desaparicion_dtf'}, inplace=True)
df['fecha_desaparicion_dtf'] = pd.to_datetime(df['fecha_desaparicion_dtf'], format='%Y').dt.year.astype(str)
df['fecha_desaparicion_dtf'] = df['fecha_desaparicion_dtf'].apply(lambda x: '' if len(x) != 4 else x)
df['fecha_desaparicion_dtf'] = df['fecha_desaparicion_dtf'].str.replace('18', '19', n=1)
df['fecha_desaparicion_dtf'] = df['fecha_desaparicion_dtf'].str.replace('179', '197', n=1)
df['fecha_desaparicion_dtf'] = df['fecha_desaparicion_dtf'].str.replace('169', '196', n=1)
df['fecha_desaparicion_dtf'] = df['fecha_desaparicion_dtf'].str.replace('159', '195', n=1)
# Crear la variable 'fecha_desaparicion' como texto concatenando las partes de la fecha
df['fecha_desaparicion'] = df['fecha_ocur_anio'] + df['fecha_ocur_mes'] + df['fecha_ocur_dia']
df['fecha_desaparicion'] = df['fecha_desaparicion'].apply(lambda x: '' if pd.isna(x) else str(x))
# Tipo de responsable
# Paramilitares
...
# Nombre completo
df['nombre_completo'] = df['primer_nombre']
for i in range(2, 5):
    df['nombre_completo'] += " " + df['v' + str(i)]  # Concatenar nombres y apellidos no vacíos
    df.drop(columns=['v' + str(i)], inplace=True)
df['nombre_completo'] = df['nombre_completo'].str.strip()  # Eliminar espacios en blanco al principio y al final
df['nombre_completo'] = df['nombre_completo'].str.replace('  ', ' ', regex=True)  # Reemplazar espacios dobles por espacios simples
...
df.rename(columns={'iden_pertenenciaetnica': 'iden_pertenenciaetnica'}, inplace=True)
# Recodificar valores
...
df_rni.to_stata("archivos depurados/BD_FGN_INACTIVOS_PNI.dta", index=False)
