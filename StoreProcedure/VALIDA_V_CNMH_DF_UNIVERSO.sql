...
/****** Object:  StoredProcedure [dbo].[VALIDA_V_CNMH_DF_UNIVERSO]    Script Date: 3/11/2023 2:35:16 p. m. ******/
DROP PROCEDURE [dbo].[VALIDA_V_CNMH_DF_UNIVERSO]
GO
...
-- Description:	Realiza la consulta de las personas desaparecidas de la fuente V_CNMH_DF
-- =============================================
CREATE   PROCEDURE [dbo].[VALIDA_V_CNMH_DF_UNIVERSO]
WITH RECOMPILE 
AS
BEGIN
  SET NOCOUNT ON;
  IF OBJECT_ID('[dbo].[BD_CNMH_DF_VAL]', 'U') IS NOT NULL
    DROP TABLE [dbo].[BD_CNMH_DF_VAL]
  ...
  SELECT A.[tabla_origen]
	  ...
	  ,CASE WHEN Coalesce(CONVERT(VARCHAR,A.[anio_nacimiento]),'Nulo') = Coalesce(CONVERT(VARCHAR,B.[anio_nacimiento]),'Nulo') THEN 'Igual' ELSE 'Diferente' END  AS anio_nacimiento_v
      ,A.[mes_nacimiento] AS mes_nacimiento_u
      ,B.[mes_nacimiento] AS mes_nacimiento_i
	  ,CASE WHEN Coalesce(CONVERT(VARCHAR,A.[mes_nacimiento]),'Nulo') = Coalesce(CONVERT(VARCHAR,B.[mes_nacimiento]),'Nulo') THEN 'Igual' ELSE 'Diferente' END  AS mes_nacimiento_v
      ,A.[dia_nacimiento] AS dia_nacimiento_u
      ,B.[dia_nacimiento] AS dia_nacimiento_i
	  ,CASE WHEN Coalesce(CONVERT(VARCHAR,A.[dia_nacimiento]),'Nulo') = Coalesce(CONVERT(VARCHAR,B.[dia_nacimiento]),'Nulo') THEN 'Igual' ELSE 'Diferente' END  AS dia_nacimiento_v
      ,A.[edad] AS edad_u
      ,B.[edad] AS edad_i
	  ...
	  ,CASE WHEN Coalesce(CONVERT(VARCHAR,A.[fecha_ocur_anio]),'Nulo') = Coalesce(CONVERT(VARCHAR,B.[fecha_ocur_anio]),'Nulo') THEN 'Igual' ELSE 'Diferente' END  AS fecha_ocur_anio_v
	  ,A.[fecha_ocur_mes] AS fecha_ocur_mes_u
	  ,B.[fecha_ocur_mes] AS fecha_ocur_mes_i
	  ,CASE WHEN Coalesce(CONVERT(VARCHAR,A.[fecha_ocur_mes]),'Nulo') = Coalesce(CONVERT(VARCHAR,B.[fecha_ocur_mes]),'Nulo') THEN 'Igual' ELSE 'Diferente' END   AS fecha_ocur_mes_v
      ,A.[fecha_ocur_dia] AS fecha_ocur_dia_u
      ,B.[fecha_ocur_dia] AS fecha_ocur_dia_i
	  ,CASE WHEN Coalesce(CONVERT(VARCHAR,A.[fecha_ocur_anio]),'Nulo') = Coalesce(CONVERT(VARCHAR,B.[fecha_ocur_anio]),'Nulo') THEN 'Igual' ELSE 'Diferente' END  AS fecha_ocur_dia_v
      ,A.[codigo_dane_departamento] AS codigo_dane_departamento_u
      ,B.[codigo_dane_departamento] AS codigo_dane_departamento_i
	  ,CASE WHEN Coalesce(CONVERT(VARCHAR,A.[codigo_dane_departamento]),'Nulo') = Coalesce(CONVERT(VARCHAR,B.[codigo_dane_departamento]),'Nulo') THEN 'Igual' ELSE 'Diferente' END  AS codigo_dane_departamento_v
      ,A.[departamento_ocurrencia] AS departamento_ocurrencia_u
      ,B.[departamento_ocurrencia] AS departamento_ocurrencia_i
	  ,CASE WHEN Coalesce(CONVERT(VARCHAR,A.[departamento_ocurrencia]),'Nulo') = Coalesce(CONVERT(VARCHAR,B.[departamento_ocurrencia]),'Nulo') THEN 'Igual' ELSE 'Diferente' END  AS departamento_ocurrencia_v
      ,A.[codigo_dane_municipio] AS codigo_dane_municipi_u
      ,B.[codigo_dane_municipio] AS codigo_dane_municipi_i
	  ,CASE WHEN Coalesce(CONVERT(VARCHAR,A.[codigo_dane_municipio]),'Nulo') = Coalesce(CONVERT(VARCHAR,B.[codigo_dane_municipio]),'Nulo') THEN 'Igual' ELSE 'Diferente' END  AS codigo_dane_municipi_v
	  ...
	  INTO [dbo].[BD_CNMH_DF_VAL]
  FROM (SELECT CONVERT(VARCHAR,personas.IdCaso) + '_' + CONVERT(VARCHAR,personas.IdentificadorCaso) + '_' + CONVERT(VARCHAR,Id)  AS codigo_unico_fuente,personas.* 
  FROM [dbo].[V_CNMH_DF] personas
left join [dbo].[V_CNMH_DF_C] casos on casos.IdCaso = personas.IdCaso) C
  LEFT JOIN [dbo].[CNMH_DF_U] A
			ON C.codigo_unico_fuente = A.[codigo_unico_fuente]
		LEFT JOIN [dbo].[CNMH_DF] B 
