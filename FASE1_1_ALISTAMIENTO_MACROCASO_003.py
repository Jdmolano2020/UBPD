...
V_JEP_MACROCASO_003_file_path = os.path.join(DIRECTORY_PATH, "archivos depurados", "V_JEP_MACROCASO_003.csv")
...
df_003['codigo_dane_municipio_'] = df_003['codigo_dane_municipio'].apply(lambda x: "{:05.0f}".format(x))
...
df_003.rename(columns={'codigo_dane_municipio_': 'codigo_dane_municipio'}, inplace=True)

# Reemplace 'codigo_dane_municipio' con "05042" cuando depto=="ANTIOQUIA" & municipio=="SANTAFE DE ANTIOQUIA"
df_003.loc[(df_003['depto'] == "ANTIOQUIA") & (df_003['municipio'] == "SANTAFE DE ANTIOQUIA"), 'codigo_dane_municipio'] = "05042"
...
df_003 = pd.merge(df_003, dane[['codigo_dane_municipio', 'municipio', 'departamento']], how='left', left_on=['codigo_dane_municipio'],
                right_on=['codigo_dane_municipio'], indicator = True).replace({'left_only': 1, 'right_only': 2, 'both': 3})
...
df_003.rename(columns={'municipio': 'municipio_ocurrencia', 'departamento': 'departamento_ocurrencia'}, inplace=True)
...
# fecha de ocurrencia 
df_003['fecha_ocur_anio'] = pd.to_numeric(df_003['fecha_ocur_anio'], errors='coerce')
df_003['fecha_ocur_mes'] = pd.to_numeric(df_003['fecha_ocur_mes'], errors='coerce')
df_003['fecha_ocur_dia'] = pd.to_numeric(df_003['fecha_ocur_dia'], errors='coerce')
...
df_003['fecha_ocur_anio'] = df_003['fecha_ocur_anio'].apply(lambda x: '' if 
    (isinstance(x, int) or isinstance(x, float)) and len(str(x)) != 4 else x)

df_003['fecha_ocur_anio'] = df_003['fecha_ocur_anio'].apply(lambda x: x.replace("18", "19", 1) if isinstance(x, str) and x.startswith("18") else x)
df_003['fecha_ocur_anio'] = df_003['fecha_ocur_anio'].apply(lambda x: x.replace("179", "197", 1) if isinstance(x, str) and x.startswith("179") else x)
df_003['fecha_ocur_anio'] = df_003['fecha_ocur_anio'].apply(lambda x: x.replace("169", "196", 1) if isinstance(x, str) and x.startswith("169") else x)
df_003['fecha_ocur_anio'] = df_003['fecha_ocur_anio'].apply(lambda x: x.replace("159", "195", 1) if isinstance(x, str) and x.startswith("159") else x)

df_003['fecha_ocur_anio_dtf'] = pd.to_numeric(df_003['fecha_ocur_anio'], errors='coerce')

df_003['fecha_desaparicion_dtf'] = df_003['fecha_desaparicion_dtf'].dt.strftime('%d')

df_003.drop(columns=['fecha_ocur_anio_dtf'], inplace=True)

# Datos sobre las personas dadas por desparecidas
# Renombre columnas
df_003.rename(columns={'seg_nombre': 'segundo_nombre', 'seg_apellido': 'segundo_apellido'}, inplace=True)
...
#4. Eliminación de Registros No Identificados (registros sin datos suficientes para la individualización de las víctimas) 
# Crear la variable 'non_miss' contando los valores no perdidos por fila
df_003['non_miss'] = df_003[['primer_nombre', 'segundo_nombre', 'primer_apellido', 'segundo_apellido']].apply(lambda row: row.apply(lambda x: x != '').sum(), axis=1)

# Crear la variable 'rni' basada en 'non_miss'
df_003['rni'] = df_003['non_miss'] < 2

df_003.loc[(df_003['codigo_dane_departamento'] == "") & (df_003['fecha_ocur_anio'] == "") & (df_003['documento'] == ""), 'rni'] = 1
...
BD_MACRO_003_PNI_file_path = os.path.join(DIRECTORY_PATH, "archivos depurados", "BD_MACRO_003_PNI.csv")
...
with engine.connect() as conn, conn.begin():
    df_003_rni.to_sql(name=DB_TABLE, con=engine, schema=DB_SCHEMA, if_exists='replace', index=False)
...
# Eliminar las columnas 'non_miss', 'N', y todas las columnas que comienzan con 'rni'
df_003.drop(columns=df_003.columns[df_003.columns.str.startswith('rni')].tolist() + ['non_miss', 'N'], inplace=True)
...
# Guardar el DataFrame ordenado en un archivo CSV (puedes ajustar el formato según tus necesidades)
BD_MACRO_003_file_path = os.path.join(DIRECTORY_PATH, "archivos depurados", "BD_MACRO_003.csv")
...
with engine.connect() as conn, conn.begin():
    df_003.to_sql(name=DB_TABLE, con=engine, schema=DB_SCHEMA, if_exists='replace', index=False)
