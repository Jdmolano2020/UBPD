USE [ubpd_base]
GO
/****** Object:  StoredProcedure [dbo].[VALIDA_RSB_UNIVERSO]    ******/
SET ANSI_NULLS ON
GO
...
-- Description:	Realiza la consulta de las personas desaparecidas de la fuente RSB
-- =============================================
ALTER   PROCEDURE [dbo].[VALIDA_RSB_UNIVERSO]
WITH RECOMPILE 
AS
BEGIN
  SET NOCOUNT ON;
     DROP TABLE IF EXISTS [UNIVERSO_PDD].[validado].[BD_RSB_VAL]
	  ...
	  INTO [UNIVERSO_PDD].[validado].[BD_RSB_VAL]
  FROM [UNIVERSO_PDD].[orq_salida].[RSB] C LEFT JOIN [UNIVERSO_PDD].[depurados].[BD_UBPD_RSB] A
			ON C.[Codigo unico fuente] = A.[codigo_unico_fuente]
		LEFT JOIN [UNIVERSO_PDD].[version5].[BD_UBPD_RSB] B 
			ON C.[Codigo unico fuente] =  B.codigo_unico_fuente

	UPDATE A SET A.campos_revisar =	CASE WHEN A.[codigo_unico_fuente_v] = 'Diferente' THEN 'codigo_unico_fuente;' ELSE '' END +
								CASE WHEN A.[nombre_completo_v] = 'Diferente' THEN 'nombre_completo;' ELSE '' END	+					
								CASE WHEN A.[primer_nombre_v] = 'Diferente' THEN 'primer_nombre;' ELSE '' END +
								CASE WHEN A.[segundo_nombre_v] = 'Diferente' THEN 'segundo_nombre;' ELSE '' END +
								CASE WHEN A.[primer_apellido_v] = 'Diferente' THEN 'primer_apellido;' ELSE '' END +
								CASE WHEN A.[segundo_apellido_v] = 'Diferente' THEN 'segundo_apellido;' ELSE '' END +
								CASE WHEN A.[documento_v] = 'Diferente' THEN 'documento;' ELSE '' END +
								CASE WHEN A.[sexo_v] = 'Diferente' THEN 'sexo;' ELSE '' END +
								CASE WHEN A.[iden_pertenenciaetnic_v] = 'Diferente' THEN 'iden_pertenenciaetnica;' ELSE '' END +
								CASE WHEN A.[fecha_nacimiento_v] = 'Diferente' THEN 'fecha_nacimiento;' ELSE '' END +
								CASE WHEN A.[anio_nacimiento_v] = 'Diferente' THEN 'anio_nacimiento;' ELSE '' END +
								CASE WHEN A.[mes_nacimiento_v] = 'Diferente' THEN 'mes_nacimiento;' ELSE '' END +
								CASE WHEN A.[dia_nacimiento_v] = 'Diferente' THEN 'dia_nacimiento;' ELSE '' END +
								CASE WHEN A.[edad_v] = 'Diferente' THEN 'edad;' ELSE '' END +
								CASE WHEN A.[fecha_desaparicion_v] = 'Diferente' THEN 'fecha_desaparicion;' ELSE '' END +
								CASE WHEN A.[fecha_ocur_anio_v] = 'Diferente' THEN 'fecha_ocur_anio;' ELSE '' END +
								CASE WHEN A.[fecha_ocur_mes_v] = 'Diferente' THEN 'fecha_ocur_mes;' ELSE '' END +
								CASE WHEN A.[fecha_ocur_dia_v] = 'Diferente' THEN 'fecha_ocur_dia;' ELSE '' END +
								CASE WHEN A.[codigo_dane_departamento_v] = 'Diferente' THEN 'codigo_dane_departamento;' ELSE '' END +
								CASE WHEN A.[departamento_ocurrencia_v] = 'Diferente' THEN 'departamento_ocurrencia;' ELSE '' END +
								CASE WHEN A.[codigo_dane_municipi_v] = 'Diferente' THEN 'codigo_dane_municipio;' ELSE '' END +
								CASE WHEN A.[municipio_ocurrencia_v] = 'Diferente' THEN 'municipio_ocurrencia;' ELSE '' END +
								CASE WHEN A.[TH_DF_v] = 'Diferente' THEN 'TH_DF;' ELSE '' END +
								CASE WHEN A.[TH_SE_v] = 'Diferente' THEN 'TH_SE;' ELSE '' END +
								CASE WHEN A.[TH_RU_v] = 'Diferente' THEN 'TH_RU;' ELSE '' END +
								CASE WHEN A.[TH_OTRO_v] = 'Diferente' THEN 'TH_OTRO;' ELSE '' END +
								CASE WHEN A.[pres_resp_paramilitares_v] = 'Diferente' THEN 'pres_resp_paramilitares;' ELSE '' END +
								CASE WHEN A.[pres_resp_grupos_posdesmov_v] = 'Diferente' THEN 'pres_resp_grupos_posdesmov;' ELSE '' END +
								CASE WHEN A.[pres_resp_agentes_estatales_v] = 'Diferente' THEN 'pres_resp_agentes_estatales;' ELSE '' END +
								CASE WHEN A.[pres_resp_guerr_farc_v] = 'Diferente' THEN 'pres_resp_guerr_farc;' ELSE '' END +
								CASE WHEN A.[pres_resp_guerr_eln_v] = 'Diferente' THEN 'pres_resp_guerr_eln;' ELSE '' END +
								CASE WHEN A.[pres_resp_guerr_otra_v] = 'Diferente' THEN 'pres_resp_guerr_otra;' ELSE '' END +
								CASE WHEN A.[pres_resp_otro_v] = 'Diferente' THEN 'pres_resp_otro;' ELSE '' END +
								CASE WHEN A.[situacion_actual_des_v] = 'Diferente' THEN 'situacion_actual_des;' ELSE '' END +
								CASE WHEN A.[descripcion_relato_v] = 'Diferente' THEN 'descripcion_relato;' ELSE '' END
						FROM [UNIVERSO_PDD].[validado].[BD_RSB_VAL] A

    SELECT campos_revisar, [tabla_origen],[codigo_unico_fuente_o],[codigo_unico_fuente_u],[codigo_unico_fuente_i],[codigo_unico_fuente_v],[nombre_completo_u],[nombre_completo_i],[nombre_completo_v]
      ,[primer_nombre_u],[primer_nombre_i],[primer_nombre_v],[segundo_nombre_u],[segundo_nombre_i],[segundo_nombre_v],[primer_apellido_u],[primer_apellido_i]
      ,[primer_apellido_v],[segundo_apellido_u],[segundo_apellido_i],[segundo_apellido_v],[documento_u],[documento_i],[documento_v],[sexo_u],[sexo_i]
      ,[sexo_v],[iden_pertenenciaetnic_u],[iden_pertenenciaetnic_i],[iden_pertenenciaetnic_v],[fecha_nacimiento_u],[fecha_nacimiento_i],[fecha_nacimiento_v]
      ,[anio_nacimiento_u],[anio_nacimiento_i],[anio_nacimiento_v],[mes_nacimiento_u],[mes_nacimiento_i],[mes_nacimiento_v],[dia_nacimiento_u],[dia_nacimiento_i]
      ,[dia_nacimiento_v],[edad_u],[edad_i],[edad_v],[fecha_desaparicion_u],[fecha_desaparicion_i],[fecha_desaparicion_v],[fecha_ocur_anio_u],[fecha_ocur_anio_i]
      ,[fecha_ocur_anio_v],[fecha_ocur_mes_u],[fecha_ocur_mes_i],[fecha_ocur_mes_v],[fecha_ocur_dia_u],[fecha_ocur_dia_i]
	,[fecha_ocur_dia_v],[codigo_dane_departamento_u],[codigo_dane_departamento_i],[codigo_dane_departamento_v],[departamento_ocurrencia_u],[departamento_ocurrencia_i],
	[departamento_ocurrencia_v],[codigo_dane_municipi_u],[codigo_dane_municipi_i],[codigo_dane_municipi_v],[municipio_ocurrencia_u],[municipio_ocurrencia_i]
	,[municipio_ocurrencia_v],[TH_DF_u],[TH_DF_i],[TH_DF_v],[TH_SE_u],[TH_SE_i],[TH_SE_v],[TH_RU_u],[TH_RU_i],[TH_RU_v]
 
	FROM [UNIVERSO_PDD].[validado].[BD_RSB_VAL]
END
